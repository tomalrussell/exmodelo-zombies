...
res <- read.csv("resultsNSGA/population4000.csv", header = T)
...
dim(df2)
sel = which(df2$fitness3 == min(df2$fitness3))
df2[sel,]
...
nbcol <- heat.colors(length(df2$fitness3))
nbcol[zcol]
z = df2$fitness3
